                                                  ...
                                                  int const frames_story, int const max_dist)
{
    ...
    for (auto &prev_bbox_vec : prev_bbox_vec_deque)
    {
        ...
        {
            int cur_index = -1;
            float curr_center_x_diff;
            float curr_center_y_diff;
            for (size_t m = 0; m < cur_bbox_vec.size(); ++m)
            {
                ...
                if (i.obj_id == k.obj_id)
                {
                    ...
                    if (cur_dist < max_dist && (k.track_id == 0 || dist_vec[m] > cur_dist))
                    {
                        dist_vec[m] = cur_dist;
                        cur_index = m;
                        curr_center_x_diff = center_x_diff;
                        curr_center_y_diff = center_y_diff;
                    }
                }
            }
            ...
            if (cur_index >= 0 && track_id_absent)
            {
                ...
                cur_bbox_vec[cur_index].story = i.story;
                center_t center;
                center.x = (float)(i.x + i.w / 2);
                center.y = (float)(i.y + i.h);
                cur_bbox_vec[cur_index].story.push_back(center);
            }
        }
    }
